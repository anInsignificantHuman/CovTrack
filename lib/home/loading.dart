...
class _GlobalLoadScreenState extends State<GlobalLoadScreen> {
  Future<void> getData() async {
    ...
    List rows = [for (var link in links) link.parent.parent];
    ...
    Navigator.push(context,
        LoadRoute(builder: (context) => Home(totals[0], totals[1], table)));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: Column(
            ...
            children: [
              SpinKitPouringHourglass(color: Colors.white, size: 80.0)
            ]));
  }
}
